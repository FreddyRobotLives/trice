...
const CACHE = 'trice-shell-v133';
const CORE = ['/', '/index.html', '/manifest.webmanifest', '/icon-192.png', '/icon-512.png', '/vendor/leaflet.min.js', '/vendor/leaflet.min.css', '/vendor/InterVariable.woff2'];
...
self.addEventListener('fetch', (e) => {
  ...
  // App shell
  if (url.pathname !== '/' && url.pathname !== '/index.html' && req.mode === 'navigate') {
      /* Standalone pages (rescue, recovery) must never be answered with the
         cached app shell. Straight to the network, no race. */
      return;
    }
