...
## v24.8 (17 Aug 2026) — CRITICAL
All third-party CDN dependencies removed. Leaflet, SheetJS and ExcelJS now
ship in /vendor and serve from trice.live itself. This fixes XLSX exports
and map failures on networks that block or throttle cdnjs.cloudflare.com.
The /vendor folder MUST be committed with this release.
Every XLSX export is stamped with the build version in the toast and on the
Summary sheet ("Generated by") so a downloaded file identifies its build.

## v25.9 (18 Aug 2026)
- Inter variable font app-wide, vendored at /vendor/InterVariable.woff2 (commit /vendor).
- Landing repositioned: "Complex processes, made simple." across hero, meta, og.png (?v=31).
- Asset Database + all photo documents now render photos by blob reference (crash fix).
- Instant loading overlay on all heavy documents; silent XLSX download (no ready bar).
- Grid photos: mobile card thumbnails + frozen desktop Photo column.
After deploy: Facebook Sharing Debugger "Scrape Again" + LinkedIn Post Inspector on trice.live.

## v26.0-v26.1 (18 Aug 2026)
- Client Asset Database PDF: client-safe copy (no notes, no pricing, no AI language,
  no crew names); document itself carries no client marking.
- Landing slogan: "The field changes fast. Your data should keep up." with
  widow-proof wrapping; og.png + meta synced (?v=32). Scrape FB/LinkedIn after deploy.
